...
@section('title', 'Edit Section - USTP DMCRS')
...
<div class="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-lg">
    ...
    <div class="text-center mb-8">
        ...
        <h2 class="text-3xl font-bold text-ustpBlue mb-2">Edit Section</h2>
        <p class="text-gray-600">Update section information</p>
    </div>
    ...
    <form action="{{ route('admin.sections.update', $section->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="year_level" id="year_level"
                ...
                @for($i = 1; $i <= 6; $i++)
                    <option value="{{ $i }}"
                            {{ (old('year_level') ?? $section->year_level) == $i ? 'selected' : '' }}>
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
            <h4 class="font-semibold text-gray-800 mb-2">Current Section:</h4>
            <p class="text-gray-700">{{ $section->full_name }}</p>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updatePreview() {
            ...
            if (departmentText && yearLevel && sectionName && departmentText !== 'Select Department') {
                ...
                previewText.textContent = `Updated Section Name: ${fullName}`;
